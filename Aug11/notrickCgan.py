...
def generator(z,reuse= None):
    with tf.variable_scope('Generator', reuse=reuse):
        ...
        net_sg= tf.contrib.layers.fully_connected(inputs=net2, num_outputs=64, activation_fn=tf.sigmoid,scope="sg") #-ve log(sigma_sq)/8
        epsilon= tf.random_normal(shape = tf.shape(net_mu),mean =0.0 ,stddev = 1.0, dtype = tf.float32 )
        sample = net_mu + 0*epsilon*tf.exp(-10*net_sg) # tf.random_normal(shape = tf.shape(net_mu),mean = net_mu,stddev = tf.sqrt(tf.exp(-net_sg)), dtype = tf.float32 )
    return sample
...
clip_D = [p.assign(tf.clip_by_value(p, -0.2, 0.2)) for p in d_param]
...
with tf.Session() as sess:
    ...
    if Is_train == True:
        ...
        e=0
        for i in range(0,80000,5):      #total data 32*5000 = 160000
    ...
    zsample = np.mgrid[0.1:2:0.05937, 0:2:0.02].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
    n_samples = (32,100)
    n = n_samples[1]
    Mdist = []
    Edist = []
    ...
    for i in range(0,32,3):
        ...
        gsample = sess.run(G, feed_dict={z_cond:np.repeat(zsample[i*n_samples[1]:(i+1)*n_samples[1],0],10).reshape(n_samples[1],10),z_rand:zsample[i*n_samples[1]:(i+1)*n_samples[1],1].reshape(n_samples[1],1) })#0.5*np.random.normal( size=[n, n_z])
