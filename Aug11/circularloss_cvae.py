...
loss_type  = 'log_gaussian'
datapoints = 320000
n_temps    = 32

fully_connected1 = tf.contrib.layers.fully_connected(inputs=tf.concat([y,x],axis=1),num_outputs=32, activation_fn=tf.nn.relu,scope="Fully_Conn1")
fully_connected2 = tf.contrib.layers.fully_connected(inputs=fully_connected1,   num_outputs=24,   activation_fn=tf.nn.relu,scope="Fully_Conn2")
...
#decoder
epsilon = 1e-10
fully_connected_decoder1 = tf.contrib.layers.fully_connected(inputs=tf.concat([y,z],axis=1), num_outputs=24, activation_fn=tf.nn.relu,scope="Fully_Conn1_decoder")
...
if loss_type == 'Binary_crossentropy':
    x_theta = tf.contrib.layers.fully_connected(inputs=fully_connected_decoder2, num_outputs=64, activation_fn=tf.sigmoid,scope="Fully_Conn2_decoder_out")
    x_hat   = tf.concat([tf.math.cos(2*np.pi*x_theta),tf.math.sin(2*np.pi*x_theta)],axis=1)
    recon_loss =-1*tf.reduce_sum( x * tf.log(epsilon+x_hat) + (1-x)*tf.log(epsilon+1-x_hat),axis=1) #
elif loss_type == 'log_gaussian':
    x_mu           = tf.contrib.layers.fully_connected(inputs=fully_connected_decoder2, num_outputs=64, activation_fn=tf.nn.relu,scope="Fully_Conn2_decoder_mu")
    ...
    x_theta        = tf.random_normal(shape = tf.shape(x_mu) ,mean = x_mu, stddev =tf.sqrt(tf.exp(-x_log_sigma_sq)), dtype = tf.float32 )
    num            = tf.math.minimum((x_mu-x)**2,tf.math.minimum((x_mu-x+1)**2,(x_mu-x-1)**2))
    # x_hat          = tf.concat([tf.math.cos(2*np.pi*x_mu),tf.math.sin(2*np.pi*x_mu)],axis=1)
    recon_loss     = 0.5*tf.reduce_sum( (num)/tf.exp(-x_log_sigma_sq)+1.837- x_log_sigma_sq ,axis=1)
# Reconstruction Loss
recon_loss = tf.reduce_mean(recon_loss)
# Latent loss
KL_loss = -3*tf.reduce_sum(    1 + z_log_sigma_sq - tf.square(z_mu) -tf.exp(z_log_sigma_sq), axis=1)
...
with tf.Session() as sess:
    ...
    sess.run(iterator.initializer, feed_dict = {m:training_data,n:np.array(tvals).reshape(datapoints,1) + 0.05*np.random.randn(datapoints,1)})
    ...
    zsample = np.mgrid[0.0:2.0:0.2, -3.0:3.0:0.3].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
    n_samples = (10,20)    # int(np.shape(zsample)[0]**(1/n_z))
    if n_z == 1:
        gsample = sess.run(x_theta, feed_dict={z: zsample[:,1].reshape(zsample.shape[0],1), y:zsample[:,0].reshape(zsample.shape[0],1) + 0.05*np.random.randn(zsample.shape[0],1)})  #np.array(tvals[:n_samples]).reshape(n_samples,1)}
