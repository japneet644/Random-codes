...
learning_rate = 1e-4
batch_size    = 100
n_z           = 10 # no of temp variables(generally 1)
...
with tf.variable_scope('loss'):
    ...
    energy_loss = tf.reduce_mean(tf.reduce_sum( (calculate_energy(x)-calculate_energy(sample))**2 , axis=1))
...
with tf.Session() as sess:
    ...
    n = 300
    zsample = 0.5*np.random.normal(size=[n,n_zrand])#np.linspace(-1.5,1.5,n).reshape(n,1)#
    Mdist = []
    Edist = []
    mean_magnetization = []
    var_magnetization = []
    ...
    if Is_train == l:
        ...
        print(tvd_mag)
        plt.errorbar(T_vals,mean_magnetization,var_magnetization,color='b',label='VAE Samples')
        plt.errorbar(T_vals,mean_magnetization_data,var_magnetization_data,color = 'g',label='MC samples')
        ...
        print(tvd_energy)
        plt.errorbar(T_vals,mean_energy,var_energy,color='b',label='VAE Samples')
        plt.errorbar(T_vals,mean_energy_data,var_energy_data,color = 'g',label='MC samples')
        plt.xlabel("Temperature")
        plt.ylabel('Energy')
        plt.title('HGVAE')
        plt.legend()
