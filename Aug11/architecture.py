...
z_temp           = tf.contrib.layers.fully_connected(inputs=fully_connected2, num_outputs= 1, activation_fn=tf.nn.relu,scope="Fully_Conn2_temp")
...
with tf.Session() as sess:
    ...
    if Is_train == True:
        ...
        tvals = np.repeat(np.linspace(0.1,2.0,32),100000)
    ...
    T_vals  = np.linspace(0.1,2.1,8)
    zsample = np.mgrid[0.1:2.1:0.25, -2.0:2.0:0.04].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
    n_samples = (8,100)
    if loss_type == 'Binary_crossentropy':
        # Gsample    = sess.run(fully_connected_decoder1, feed_dict={z: zsample[:,1].reshape(zsample.shape[0],1), z_temp:zsample[:,0].reshape(n_samples[1],1) + 0.01*np.random.randn(n_samples[1],1)})
        ...
        Gsample    = sess.run(fully_connected_decoder1, feed_dict={z: zsample[:,1].reshape(zsample.shape[0],1), z_temp:zsample[:,0].reshape(zsample.shape[0],1) })  #np.random.normal(size = [n_samples[1]*n_samples[0],n_z])
        Gsample2   = sess.run(fully_connected_decoder2, feed_dict={fully_connected_decoder1: Gsample})
        ...
        gsample    = sess.run(tf.random_normal(shape = tf.shape(Gsamplemu),mean = Gsamplemu,stddev = 0*tf.sqrt(tf.exp(-Gsamplesig)), dtype = tf.float32 ))#tf.sqrt(tf.exp(Gsamplesig))
    ...
    for i in range(0,n_samples[0]):
        ...
        print(i,np.mean(gsample[i*n_samples[1]]),np.std(gsample[i*n_samples[1]]))
        if Is_train == a:
            fig1 = plt.figure(1)
