...
datapoints = 320000
n_temps    = 32
T_vals = np.linspace(-1,0.9,n_temps)
...
def generator(z,reuse= None):
    with tf.variable_scope('Generator', reuse=reuse):
        net   = tf.contrib.layers.fully_connected(inputs=z   , num_outputs=32, activation_fn=tf.nn.relu,scope="inp")
        net2  = tf.contrib.layers.fully_connected(inputs=net , num_outputs=128,activation_fn=tf.nn.relu,scope="hid")
        net_mu= tf.contrib.layers.fully_connected(inputs=net2, num_outputs=64, activation_fn=tf.tanh,scope="mu")
        net_sg= tf.contrib.layers.fully_connected(inputs=net2, num_outputs=64, activation_fn=tf.tanh,scope="sg") #-ve log(sigma_sq)/8
...
def discriminator(g,reuse = None):
    with tf.variable_scope('Discriminator', reuse=reuse):
        net   = tf.contrib.layers.fully_connected(inputs=g   , num_outputs=128, activation_fn=tf.nn.relu  ,scope="inp")
        net2  = tf.contrib.layers.fully_connected(inputs=net , num_outputs=64,  activation_fn=tf.nn.relu  ,scope="hid")
        d_out = tf.contrib.layers.fully_connected(inputs=net2, num_outputs=1 ,  activation_fn=None     ,scope="prob")
        r1    = tf.contrib.layers.fully_connected(inputs=net2, num_outputs=24,  activation_fn=tf.tanh  ,scope="hid2")
        r_cat = tf.contrib.layers.fully_connected(inputs=r1  ,num_outputs=n_zcat,activation_fn=None    ,scope="cat")
        r_mu  = tf.contrib.layers.fully_connected(inputs=r1  ,num_outputs=n_zc, activation_fn=tf.tanh  ,scope="mu")
        r_sg  = tf.contrib.layers.fully_connected(inputs=r1  ,num_outputs=n_zc, activation_fn=tf.tanh  ,scope="sg")
    return d_out,r_cat,r_mu,r_sg

def loss_func(logits_in,labels_in):
    return tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_in,labels=labels_in))

G = generator(z)
d_real,_,_,_ = discriminator(g=tf.concat([x,z_cat],axis=1))
d_fake, r_cat, r_mu, r_sg = discriminator(g=tf.concat([G,z_cat],axis=1),reuse = True)

# 3. Calculate loss
# -log(D(G(x))) trick
with tf.variable_scope('loss'):
    g_loss   = loss_func(d_fake,tf.ones_like(d_fake))
    cat_loss = tf.reduce_mean(tf.reduce_sum((r_cat-z_cat)**2, axis=1))#
    cont_loss = 0.5*tf.reduce_mean(tf.reduce_sum(((z_cont-r_mu)**2)/tf.sqrt(tf.exp(-5*r_sg))-5*r_sg, axis=1))
    train_g_loss = g_loss + 0.1*cat_loss+0.1*cont_loss

    d_loss_fake = loss_func(d_fake,tf.zeros_like(d_real))
    d_loss_real = loss_func(d_real,tf.ones_like(d_real)*0.9)
    train_d_loss = d_loss_fake + d_loss_real + 0.1*cat_loss+0.1*cont_loss
...
with tf.Session() as sess:
    ...
    if Is_train == True:
        ...
        for i in range(40000):      #total data 32*5000 = 160000
            if i>0 and i % (datapoints // batch_size) == 0:
                sess.run(iterator.initializer, feed_dict = {m:training_data, n:np.array(tvals).reshape(datapoints,1)+ 0.01*np.random.randn(datapoints,1) })
            g,h = sess.run(next)

            for _ in range(3):
                z_r = np.random.normal( size=[batch_size, n_zc])
                z_c = np.random.uniform(-1, 1, size=[batch_size, n_zr])
    ...
    zsample = np.mgrid[-1:1:0.1, -1:0:0.05].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
    n_samples = (20,20)
    n = 400
    Mdist = []
    Edist = []
    ...
    for i in range(0,32):
        ...
        gsample = sess.run(G, feed_dict={z_cat:z[:,0].reshape(n,1),z_cont:z[:,2].reshape(n,1),z_rand:z[:,1].reshape(n,1)})
        gsample = gsample.reshape(n,lattice_size,lattice_size)
        print(T_vals[i],360*np.mean(gsample),360*np.std(gsample))
        ...
        if Is_train == True:
            fig1 = plt.figure(1)
            plt.plot(Magnetization[0][0],label = T_vals[i]+1.1) #all values
            fig2 = plt.figure(2)
            plt.plot(Magnetization_direction,label = (T_vals[i]+1.1))
        if Is_train == False:
    ...
    if Is_train == False:
        ...
        plt.errorbar(T_vals+1.1,mean_magnetization,var_magnetization,color='b',label='Samples',alpha=0.5)
        plt.errorbar(T_vals+1.1,mean_magnetization_data,var_magnetization_data,color = 'g',label='Data',alpha=0.5)
        plt.xlabel("Temperature")
        plt.ylabel('Magnetization')
        plt.legend()
        plt.savefig('../../Desktop/InfoGAN-Magnetization.png', bbox_inches='tight')
        plt.title('InfoGAN')
        plt.show()
        ...
        plt.errorbar(T_vals+1.1,np.array(mean_energy)/64,np.array(var_energy)/64,color='b',label='Samples',alpha=0.5)
        plt.errorbar(T_vals+1.1,np.array(mean_energy_data)/64,np.array(var_energy_data)/64,color = 'g',label='Data',alpha=0.5)
        plt.xlabel("Temperature")
        plt.ylabel('Energy')
        plt.title('InfoGAN')
        plt.legend()
        plt.savefig('../../Desktop/scaledInfoGAN-Energy.png', bbox_inches='tight')
        plt.show()

        print("Specfic Heat")
        plt.plot(T_vals+1.1,(np.array(var_energy)**2)/((1.1+T_vals)**2),color='b',label='Samples',alpha=0.5)
        plt.plot(T_vals+1.1,(np.array(var_energy_data)**2)/((1.1+T_vals)**2),color='g',label='Data',alpha=0.5)
        plt.ylabel("Specific Heat")
        plt.xlabel('Temperature')
        plt.title('InfoGAN')
        plt.legend()
        plt.savefig('../../Desktop/InfoGAN-Specific Heat.png', bbox_inches='tight')
        plt.show()

        print("Magnetic Susceptibility")
        plt.plot(T_vals+1.1,(np.array(var_magnetization)**2)/(T_vals+1.1),color='b',label='Samples',alpha=0.5)
        plt.plot(T_vals+1.1,(np.array(var_magnetization_data)**2)/(T_vals+1.1),color='g',label='Data',alpha=0.5)
        plt.ylabel("Magnetic Susceptibility")
        plt.xlabel('Temperature')
        plt.title('InfoGAN')
        plt.legend()
        plt.savefig('../../Desktop/InfoGAN-Magnetic_Susceptibility.png', bbox_inches='tight')
        plt.show()
