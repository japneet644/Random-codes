...
# Latent loss
KL_loss = -5 * tf.reduce_sum(    1 + z_log_sigma_sq - tf.square(z_mu) -tf.exp(z_log_sigma_sq), axis=1)
...
with tf.Session() as sess:
    ...
    if n_z == 1:
        ...
        zsample = np.mgrid[0:2.0:0.1, -1:1.0:0.1].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
        n_samples = (20,20)    # int(np.shape(zsample)[0]**(1/n_z))
        print(n_samples)
    # if n_z == 2:
    #     zsample =  np.mgrid[-2:2.5:0.5, -2:3:0.5]
    #     n_samples = (10 ,10)
    #     zsample = zsample.reshape(2,-1).T
        if loss_type == 'Binary_crossentropy':
            Gsample    = sess.run(fully_connected_decoder1, feed_dict={z: zsample[:,1].reshape(zsample.shape[0],1), y:zsample[:,0].reshape(zsample.shape[0],1)-1})
            ...
            Gsample    = sess.run(fully_connected_decoder1, feed_dict={z: zsample[:,1].reshape(zsample.shape[0],1), y:zsample[:,0].reshape(zsample.shape[0],1)-1}) #np.array(tvals[:n_samples]).reshape(n_samples,1)}
            Gsample2   = sess.run(fully_connected_decoder2, feed_dict={fully_connected_decoder1: Gsample})
    ...
    if n_z == 1:
        ...
        plt.legend()
        plt.show()
        for d in range(0,n_samples[1],2):
            plt.plot(zsample[d:zsample.shape[0]:n_samples[1],0],Magnetization[0][0][d:zsample.shape[0]:n_samples[1]])
        plt.show()
    elif n_z == 2:
