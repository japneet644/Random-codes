...
# Latent loss
KL_loss = -0.01 * tf.reduce_sum(    1 + z_log_sigma_sq - tf.square(z_mu) -tf.exp(z_log_sigma_sq), axis=1)
...
with tf.Session() as sess:
    ...
    a = tf.placeholder(tf.float32,[datapoints, 64])
    ...
    if n_z == 1:
        ...
        n_samples = 500      # int(np.shape(zsample)[0]**(1/n_z))
