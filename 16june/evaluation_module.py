...
#
# fully_connected1 = tf.contrib.layers.fully_connected(inputs=tf.concat([y,x],axis=1), num_outputs=100, activation_fn=tf.nn.relu,scope="Fully_Conn1")
...
# # Latent loss
# KL_loss = - 10* tf.reduce_sum(    1 + z_log_sigma_sq - tf.square(z_mu) -tf.exp(z_log_sigma_sq), axis=1)
...
with tf.Session() as sess:
    ...
    for j in range(n_temps):
        zsample = abs(np.random.randn(n_samples,1))+0.8 #sess.run(z, feed_dict={x: np.array(training_data[10000*j: 10000*j+500]).reshape(500,64),y:np.array(tvals[10000*j: 10000*j+500]).reshape(500,1)}) #
        if loss_type == 'Binary_crossentropy':
            # not corrected code is wrong here
            Gsample    = sess.run(fully_connected_decoder1, feed_dict={z: zsample, y:zsample[:,0].reshape(zsample.shape[0],1)})
            ...
            Gsample    = sess.run(fully_connected_decoder1, feed_dict={z: zsample, y: np.array(tvals[j*10000:j*10000+n_samples]).reshape(n_samples,1)} )#np.array(tvals[:n_samples]).reshape(n_samples,1)}
            Gsample2   = sess.run(fully_connected_decoder2, feed_dict={fully_connected_decoder1: Gsample})
