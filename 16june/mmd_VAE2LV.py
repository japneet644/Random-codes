...
with tf.Session() as sess:
    ...
    for i in range(0000):
        if i>0 and i % (datapoints // batch_size) == 0:
            sess.run(iterator.initializer,         feed_dict = {m:training_data})
        g= sess.run(next)
        _, Losses = sess.run([train_op, losses],feed_dict={x: g})
        if i%1000==0:
            print(Losses)
    ...
    if n_z == 1:
        ...
        zsample   = np.mgrid[0.0:2.4:0.075, -3.0:3.0:0.2].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
        n_samples = (32,30)    # int(np.shape(zsample)[0]**(1/n_z))
    ...
    if n_z == 2:
        zsample =  np.mgrid[2:-2:-0.01, 2.5:-1.5:-0.125 ].reshape(2,-1).T
        n_samples = (400,32)
        if loss_type == 'Binary_crossentropy':
    ...
    if n_z == 1:
        ...
        for d in range(0,32):
            sampledz = sess.run(z,feed_dict={x:np.array(training_data[10000*d+500:10000*d+500+n_maps]).reshape(n_maps,64)})
            plt.plot(sampledz[:,0],sampledz[:,1],linestyle='dotted',label = d)
            print(np.mean(sampledz[:,0]),np.mean(sampledz[:,1]),np.mean(np.sqrt(sampledz[:,0]**2 + sampledz[:,1]**2)))
        plt.legend()
        plt.show()
        # for u in range(5,10):
        ...
        for j in range(n_samples[1]):
            print(360*np.mean(Gsamplemu[j::n_samples[1]]),360*np.std(Gsamplemu[j::n_samples[1]]))
            print(360*np.mean(-1*Gsamplesig[j::n_samples[1]]),360*np.std(-1*Gsamplesig[j::n_samples[1]]))
            print("Mean magnetization and its Standard Deviation")

            mean_magnetization = []
            Magnetization = get_parameters.get_mean_magnetization(gsample[j::n_samples[1]])
            Magnetization_direction = get_parameters.get_magnetization_direction(gsample[j::n_samples[1]])
            energy = get_parameters.get_energy(gsample[j::n_samples[1]])
            # print(Magnetization)
            temp = T_vals[j]
            lattices = np.array(training_data[j*10000:j*10000+n_samples[0]]).reshape(n_samples[0],lattice_size,lattice_size)
            ...
            # sp_heat.a ppend(get_specific_heat(lattices,temp))
