...
learning_rate = 1e-4
batch_size    = 100
n_z           = 5 # no of temp variables(generally 1)
...
def generator(z,reuse= None,bs = batch_size):
    with tf.variable_scope('Generator', reuse=reuse):
        net   = tf.contrib.layers.fully_connected(inputs=z     ,num_outputs=32, activation_fn=tf.tanh,scope="inp")
        ...
        print(Conv3)
        Convmu= tf.contrib.layers.conv2d_transpose(inputs=Conv3,num_outputs=1 , kernel_size=[3,3], stride=1, padding='VALID', activation_fn=tf.tanh, scope='Conv2mu') #(?,6,6,20) ->(?,8,8,1)
        print(Convmu)
        Convsg= tf.contrib.layers.conv2d_transpose(inputs=Conv3,num_outputs=1 , kernel_size=[3,3], stride=1, padding='VALID', activation_fn=tf.nn.relu,scope='Conv2sg') #(?,6,6,20) ->(?,8,8,1)
        print(Convsg)
...
def discriminator(g,reuse = None):
    with tf.variable_scope('Discriminator', reuse=reuse):
        ...
        input = tf.concat([padded_inp,y], axis=-1)
        print(input)
...
with tf.variable_scope('lossG'):
    train_g_loss = loss_func(d_fake,tf.ones_like(d_fake)) # Generator Loss

with tf.variable_scope('lossD'):
    D_real_loss=loss_func(d_real,tf.ones_like(d_real)*0.9) #Discriminator loss on positive samples #Smoothing for generalization
    D_fake_loss=loss_func(d_fake,tf.zeros_like(d_real)) #Discriminator loss on negative samples
...
with tf.Session() as sess:
    ...
    if Is_train == True:# for training the model
        ...
        for i in range(3):
            ...
            for _ in range(2):
                z_c = np.random.uniform(-1, 1, size=[batch_size, n_zrand])#np.random.normal(size=[batch_size, n_zrand])
                D_loss_curr,_ = sess.run([train_d_loss,d_optim], feed_dict={ x:g, z_rand:z_c,y:h,temp:j })
    ...
    if Is_train == k:
        # plt.plot(zsample,Magnetization_direction,label = (B_vals[i]+1.0))
    ...
    if Is_train == l:
        ...
        plt.errorbar(B_vals,mean_magnetization,var_magnetization,color='b',label='Samples')
        plt.errorbar(B_vals,mean_magnetization_data,var_magnetization_data,color = 'g',label='Data')
        ...
        plt.errorbar(B_vals,mean_energy,var_energy,color='b',label='Samples')
        plt.errorbar(B_vals,mean_energy_data,var_energy_data,color = 'g',label='Data')
        plt.xlabel("Temperature")
        plt.ylabel('Energy')
        plt.title('C-GAN')
        plt.legend()
