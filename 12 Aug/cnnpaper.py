...
x = tf.placeholder(name='x', dtype=tf.float32, shape=[None, lattice_size,lattice_size,2])
temp = tf.placeholder(name='temp', dtype=tf.float32, shape=[None, n_z])
y = tf.placeholder(name='y', dtype=tf.float32, shape=[None, lattice_size,lattice_size,1])
...
with tf.variable_scope('Encoder'):
    Conv1 = tf.contrib.layers.conv2d(inputs=x, num_outputs=16, kernel_size=[3, 3], stride=1, padding='SAME', activation_fn=tf.tanh)#(?,16,16,2) -> (?,16,16,16)
    print(Conv1)
...
def calculate_energy(lattice):
    ...
    H_matrix_gpu = -1*(tf.math.cos(2*np.pi*(xp-lattice)) + tf.math.cos(2*np.pi*(xn-lattice))+ tf.math.cos(2*np.pi*(yp-lattice)) + tf.math.cos(2*np.pi*(yn-lattice)))
    energy  = tf.reduce_sum((H_matrix_gpu),axis =[1,2])/(lattice_size**2)
    return energy

with tf.variable_scope('loss'):
    # Reconstruction Loss
    recon_loss = 0.5*tf.reduce_sum( (1-tf.math.cos(x[0]-Convmu))/tf.exp(-5*Convsg) + 1.837 -5*Convsg ,axis=1) #1.837= ln(2*pi)
    recon_loss = tf.reduce_mean(recon_loss)
    # Latent loss
    KL_loss = -0.5*tf.reduce_sum( 1 + Z_sg - tf.square(Z_mu) - tf.exp(Z_sg), axis=1)
    KL_loss = tf.reduce_mean(KL_loss)
    energy_loss = tf.reduce_mean(tf.reduce_sum( (calculate_energy(x[0])-calculate_energy(sample))**2 , axis=1))

total_loss = recon_loss + KL_loss + energy_loss
...
with tf.Session() as sess:
    ...
    if Is_train == True:
        ...
        m = tf.placeholder(tf.float32,[datapoints, lattice_size, lattice_size,2])
        b = tf.placeholder(tf.float32,[datapoints, n_z])
        # Uploading the data to prevent memeory issues prefetch and batching
        dataset = tf.data.Dataset.from_tensor_slices((m,b))
        ...
        print("==========< Model DELETED >===========")
        sess.run(iterator.initializer,feed_dict = {m:np.concatenate([training_data,np.repeat(np.array(bvals),(lattice_size)**2).reshape(datapoints,lattice_size,lattice_size,1)], axis = -1), b:np.repeat(np.array(bvals),n_z).reshape(datapoints,n_z) })
        ...
        for i in range(4):
            if i>0 and i % (datapoints // batch_size) == 0:
                sess.run(iterator.initializer, feed_dict = {m:np.concatenate([training_data,np.repeat(np.array(bvals),(lattice_size)**2).reshape(datapoints,lattice_size,lattice_size,1)], axis = -1), b:np.repeat(np.array(bvals),n_z).reshape(datapoints,n_z) })
            g,j = sess.run(next)
            _, Losses = sess.run([train_op, losses],feed_dict={x: g, temp:j })
            if i%1000==0:
                print(Losses)
    ...
    n = 30
    zsample = np.random.normal(size=[n,n_zrand])#np.linspace(-1.5,1.5,n).reshape(n,1)#
    Mdist = []
    Edist = []
    mean_magnetization = []
    var_magnetization = []
