...
Ehidden2 = tf.contrib.layers.fully_connected(inputs=Ehidden1, num_outputs=32, activation_fn=tf.nn.relu,scope="Fully_Conn2")
z_mu             = tf.contrib.layers.fully_connected(inputs=Ehidden2, num_outputs=n_z, activation_fn=None,scope="Fully_Conn3_mu")
...
#decoder
Dhidden1 = tf.contrib.layers.fully_connected(inputs=z, num_outputs=32, activation_fn=tf.nn.relu,scope="Fully_Conn1_decoder")
...
x_hat =    tf.contrib.layers.fully_connected(inputs=Dhidden2, num_outputs=256, activation_fn=tf.nn.sigmoid,scope="Fully_Conn3_decoder")
...
# print(x_hat)# Reconstruction Loss
epsilon = 1e-6
recon_loss = -1*tf.reduce_sum(    x * tf.log(epsilon+x_hat) +(1-x) * tf.log(epsilon+1-x_hat),axis=1)#0.5*tf.reduce_sum( ((x-x_mu)**2)/tf.exp(x_log_sigma_sq),axis=1 )
...
with tf.Session() as sess:
    saver.restore(sess,'./VAE_xy_16by16.ckpt')
    # training_data = data_loader.load_data_wrapper()
    # random.shuffle(training_data)
    # a = tf.placeholder(tf.float32,[20000, 256])
    ...
    for i in range(0000):
            # print(i)
        if i>0 and i % (20000 // batch_size) == 0:
            sess.run(iterator.initializer, feed_dict = {a:training_data})
        b = sess.run(next)
        _, Losses = sess.run([train_op, losses],feed_dict={x: b })
        if i%1000==0:
            print(Losses)
    # save_path = saver.save(sess, "./VAE_xy_16by16.ckpt")
    # print("Model saved in path: %s" % save_path)
    if n_z == 1:
        T_vals = np.linspace(0.01,2.5,20)
        n_samples = 30      # int(np.shape(zsample)[0]**(1/n_z))
        zsample = np.linspace(-5,3,n_samples).reshape(n_samples,1) #np.random.random([10,n_z])
    if n_z == 2:
        zsample =  np.mgrid[-1:2:1, -1:1:0.2]
        n_samples = (3 ,10)
        zsample = zsample.reshape(2,-1).T

    Gsample  = sess.run(Dhidden1, feed_dict={z: zsample})
    Gsample2 = sess.run(Dhidden2, feed_dict={Dhidden1: Gsample})
    gsample  = sess.run(x_hat,    feed_dict={Dhidden2:Gsample2})
    print(gsample.shape)
