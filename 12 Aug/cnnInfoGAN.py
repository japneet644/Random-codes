...
learning_rate = 1e-4
batch_size    = 20
n_z           = 1 # no of temp variables(generally 1)
n_zrand       = 2 # no of noise variables
...
def generator(z,reuse= None,bs = batch_size):
    with tf.variable_scope('Generator', reuse=reuse):
        ...
        # print(Conv1)
        Convmu= tf.contrib.layers.conv2d_transpose(inputs=Conv1,num_outputs=1 , kernel_size=[3,3], stride=1, padding='VALID', activation_fn=tf.tanh, scope='Conv2mu') #(?,1,2,2) ->(?,6,6,1)
        # print(Convmu)
        ...
        sample = Convmu + epsilon*tf.sqrt(tf.exp(-10*Convsg))#tf.random_normal(shape = tf.shape(net_mu),mean =net_mu ,stddev = tf.sqrt(tf.exp(-net_sg)), dtype = tf.float32 )#
    return sample
...
with tf.name_scope('noise_sample'):
    z_rand = tf.placeholder(name='z_r', dtype=tf.float32, shape = [None, 1])
