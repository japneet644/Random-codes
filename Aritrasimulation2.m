...
l1w1t1 = [0.505,0.505,0.505,0.501];
...
for t = 1:T
    ...
    V(t,:) = sharedprior;
%%%% first local bayesian update then sharing of beliefs and aggregation 
    
    for n = 1:4
        if signal(n) == 0
            sharedprior2(n) = sharedprior2(n)*l1w1t1(n)/(sharedprior2(n)*l1w1t1(n) + (1-sharedprior2(n))*l1w1t2(n));
        else 
            sharedprior2(n) = sharedprior2(n)*(1-l1w1t1(n))/(sharedprior2(n)*(1-l1w1t1(n)) + (1-sharedprior2(n))*(1-l1w1t2(n)));
        end
    end
    oldsharedprior = sharedprior2;
    dummyvar = [oldsharedprior(1:4)];
    sharedprior2(1) = min(dummyvar )/(min(dummyvar ) + min(1-dummyvar ));
    dummyvar = [oldsharedprior(2),oldsharedprior(1)];
    sharedprior2(2) = min(dummyvar)/(min(dummyvar) + min(1-dummyvar));

    dummyvar = [oldsharedprior(3),oldsharedprior(1)];
    sharedprior2(3) = min(dummyvar)/(min(dummyvar) + min(1-dummyvar));
    dummyvar = [oldsharedprior(4),oldsharedprior(1)];
    sharedprior2(4) = min(dummyvar)/(min(dummyvar) + min(1-dummyvar));
    W(t,:) = sharedprior2;
%%%% first aggregation and THEN LOCAL BAYESIAN UPDATE
