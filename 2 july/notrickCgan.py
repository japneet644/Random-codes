...
def generator(z,reuse= None):
    with tf.variable_scope('Generator', reuse=reuse):
        ...
        net_sg= tf.contrib.layers.fully_connected(inputs=net2, num_outputs=64, activation_fn=None,scope="sg") #-ve log(sigma_sq)/8
        epsilon= tf.random_normal(shape = tf.shape(net_mu),mean =0.0 ,stddev = 1.0, dtype = tf.float32 )
        sample = net_mu + epsilon*net_sg**2 # tf.random_normal(shape = tf.shape(net_mu),mean = net_mu,stddev = tf.sqrt(tf.exp(-net_sg)), dtype = tf.float32 )
    return sample
...
with tf.Session() as sess:
    ...
    zsample = np.mgrid[0:2:0.05937, -1:1:0.04].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
    n_samples = (32,50)
    n = 50
    Mdist = []
    Edist = []
    ...
    for i in range(0,32,3):
        ...
        gsample = sess.run(G, feed_dict={z_cond:np.repeat(zsample[i*n_samples[1]:(i+1)*n_samples[1],0],10).reshape(n_samples[1],10),z_rand: 0.5*np.random.normal( size=[n, n_z])})#zsample[i*n_samples[1]:(i+1)*n_samples[1],1].reshape(n_samples[1],1)
