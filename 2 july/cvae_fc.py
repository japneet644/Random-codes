...
fully_connected1 = tf.contrib.layers.fully_connected(inputs=tf.concat([y,x],axis=1), num_outputs=128, activation_fn=tf.nn.relu,scope="Fully_Conn1")
fully_connected2 = tf.contrib.layers.fully_connected(inputs=fully_connected1, num_outputs=64, activation_fn=tf.nn.relu,scope="Fully_Conn2")
fully_connected3 = tf.contrib.layers.fully_connected(inputs=fully_connected2, num_outputs=20, activation_fn=tf.nn.relu,scope="Fully_Conn3")
...
fully_connected_decoder2 = tf.contrib.layers.fully_connected(inputs=fully_connected_decoder1,num_outputs=64, activation_fn=tf.nn.relu,scope="Fully_Conn2_decoder")
fully_connected_decoder3 = tf.contrib.layers.fully_connected(inputs=fully_connected_decoder2,num_outputs=128,activation_fn=tf.nn.relu,scope="Fully_Conn3_decoder")
...
if loss_type == 'Binary_crossentropy':
    ...
    x_log_sigma_sq       = tf.contrib.layers.fully_connected(inputs=fully_connected_decoder3, num_outputs=64, activation_fn=tf.nn.relu,scope="Fully_Conn2_decoder_std")
    recon_loss           = 0.5*tf.reduce_sum( ((x-x_mu)**2)/tf.exp(-8*x_log_sigma_sq)+1.837- 8*x_log_sigma_sq ,axis=1) #1.837= ln(2*pi)
...
with tf.Session() as sess:
    ...
    if n_z == 1:
        zsample = np.mgrid[-1.0:0.9:0.059375, -3.0:-1.0:0.02].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
        n_samples = (32,100)
        if loss_type == 'Binary_crossentropy':
            ...
            gsample    = sess.run(tf.random_normal(shape = tf.shape(Gsamplemu),mean = Gsamplemu,stddev = tf.sqrt(tf.exp(-8*Gsamplesig)), dtype = tf.float32 ))#tf.sqrt(tf.exp(Gsamplesig))
    ...
    if Is_train == False:
        ...
        plt.title('C_VAE')
        plt.legend()
        plt.savefig('../../Desktop/C_VAE-Magnetization.png', bbox_inches='tight')
        plt.show()
        ...
        plt.title('C_VAE')
        plt.legend()
        plt.savefig('../../Desktop/C_VAE-Energy.png', bbox_inches='tight')
        plt.show()
        ...
        plt.title('C_VAE')
        plt.legend()
        plt.savefig('../../Desktop/C_VAE-Specific Heat.png', bbox_inches='tight')
        plt.show()
        ...
        plt.title('C_VAE')
        plt.legend()
        plt.savefig('../../Desktop/C_VAE-Magnetic_Susceptibility.png', bbox_inches='tight')
        plt.show()
