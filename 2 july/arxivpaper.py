...
with tf.variable_scope('Decoder'):
    ...
    if loss_type == 'Binary_crossentropy':
        ...
        recon_loss           = 0.5*tf.reduce_sum( ((x-x_mu)**2)/tf.exp(-8*x_log_sigma_sq)+1.837- 8*x_log_sigma_sq ,axis=1) #

epsilon = tf.random_normal(shape=tf.shape(x_mu),mean=0, stddev=1, dtype=tf.float32)
x_hat = x_mu + epsilon*tf.sqrt(tf.exp(-8*x_log_sigma_sq)) #tf.random_normal(shape = tf.shape(x_mu) ,mean = x_mu, stddev =tf.sqrt(tf.exp(-8*x_log_sigma_sq)), dtype = tf.float32 )
...
def calculate_energy(lattice):
    ...
    energy  = tf.reduce_sum((H_matrix_gpu),axis =[1,2])/128
    return energy
...
# print()
energy_loss = tf.reduce_sum( (calculate_energy(tf.reshape(x,shape = [-1,8,8]))-calculate_energy(tf.reshape(x_hat,shape = [-1,8,8])))**2)
...
with tf.Session() as sess:
    ...
    if n_z == 1:
        ...
        zsample = np.mgrid[-1:0.9:0.059375, -2.0:2.0:0.01].reshape(2,-1).T #np.linspace(-5,3,n_samples).reshape(n_samples,1)
        n_samples = (32,400)
        if loss_type == 'Binary_crossentropy':
